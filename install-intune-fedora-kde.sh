...
# ---------------------------------------------------------------------------
log "Step 0 — Microsoft repo"
# ---------------------------------------------------------------------------
REPO_FILE="/etc/yum.repos.d/microsoft-rhel9.0-prod.repo"
if [[ -f "$REPO_FILE" ]] && grep -q '^\[' "$REPO_FILE" 2>/dev/null; then
    ok "Repo already configured, skipping."
else
    rm -f "$REPO_FILE"*  # clean up any bad/partial repo file from a prior attempt
...
# ---------------------------------------------------------------------------
log "Step 1 — Legacy WebKit / JavaScriptCore (install-time blocker)"
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
log "Step 3 — Font/decoder libraries"
# ---------------------------------------------------------------------------
dnf install -y woff2 libmanette
ok "woff2 / libmanette present."

# ---------------------------------------------------------------------------
log "Step 4 — Isolated ICU 73 (symbol lookup fix)"
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
log "Step 5 — Image codec symlinks (version-detected, not hardcoded)"
# ---------------------------------------------------------------------------
link_codec() {
...
# ---------------------------------------------------------------------------
log "Step 6 — Verify link resolution"
# ---------------------------------------------------------------------------
...
if [[ -n "$MISSING" ]]; then
    ...
    echo "$MISSING"
    warn "Check 'dnf provides */<libname>' for each — if Fedora still packages it, dnf install it directly; if not, repeat Step 4's extract-to-${LEGACY_LIB_DIR} method."
else
    ok "All WebKit/JavaScriptCore dependencies resolved."
fi

# ---------------------------------------------------------------------------
log "Step 7 — Persistence: alias + desktop launcher"
# ---------------------------------------------------------------------------
